...
function WritePage() {
  ...
  const handleSubmit = () => {
    ...
    const newReview = {
      ...
      tags: selectedTags,
      image: imagePreviews[0] || null, // 첫 번째 이미지를 대표 사진으로
      date: new Date().toLocaleDateString()
    };

    const savedReviews = JSON.parse(localStorage.getItem('myReviews') || '[]');
    // 새 리뷰를 배열의 맨 앞에 추가 (Unshift 효과)
    localStorage.setItem('myReviews', JSON.stringify([newReview, ...savedReviews]));
        ...
        <div className="image-upload-wrapper" style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
          ...
          {imagePreviews.map((preview, index) => (
            <div key={index} className="preview-container" style={{ position: 'relative' }}>
              ...
              <button type="button" className="delete-image-btn" onClick={() => handleDeleteImage(index)}>❌</button>
            </div>
          ))}
        </div>
      </div>
